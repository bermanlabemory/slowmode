"""Supp. Fig. S6 (worms): time-evolving effective landscape on the slow mode.
...
Two panels:
  ...
  B: Barrier height Delta_V_b(t) per basin in 3-min sliding windows. The
     run barrier (escape from run basin) grows over time, consistent with the
     adaptation-driven landscape of Costa et al. (2024).

No Plot C (would require a full Langevin simulation with friction; deferred).
...
# Per-cluster behavioral content (phase velocity from the kinematic plane)
