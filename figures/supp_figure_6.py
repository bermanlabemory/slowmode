"""Fly methodology supplementary figure — 8 panels (2 rows x 4 cols).

The old panel F (participation-ratio bar comparison) has been promoted to
main Fig 4 panel D (with leave-one-fly-out SEM error bars added).  The
remaining panels are renumbered.

Layout (2 × 4):
  Row 1:
    A. PCA cumulative variance vs PC index (selects 15 PCs)
    B. Cao's E_1(d) saturation (selects d = 12)
    C. Entropy gap Δh(N) (selects N = 1200)
    D. Multi-timescale |λ_k(τ)| vs lag
  Row 2:
    E. Fixed-timescale |λ_k(τ)| vs lag
    F. τ sweep: crispness, gap |λ_5|/|λ_6|, vest count vs τ (was G)
    G. M sweep at τ = 2 s: basin sizes (sorted) vs M (was H)
    H. Held-out CV PI per transition vs M (leave-one-fly-out) (was I)
"""
import os, sys
import numpy as np
...
# ------ G: M sweep at τ=2s, basin sizes (was H) ------
