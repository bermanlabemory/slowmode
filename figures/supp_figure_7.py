"""Fly supp — reproducibility / individuality (Fig. S4).

Two panels:

  A. Per-fly G-PCCA refit cosines vs pooled, occupancy-filtered.
  B. Per-fly arm occupancy stacked bars.

(The Hernandez subspace-alignment panel was dropped: at M=4, the
chance alignment of two random 2D subspaces in 4D ambient space is
itself substantial, and the test cannot reliably distinguish observed
from a window→fly permutation null.  See compute_fig5_E_alignment_tau2s.py
for the underpowered version, kept for reference but not displayed.)
"""
